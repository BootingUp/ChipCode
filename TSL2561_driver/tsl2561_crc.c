...
void tsl2561_sig(int sig)
{
    uint8_t data[4] = { 0 };
    read(fd, data, 4);
	printf("SIG %u \n", calculateLux(0, 3, data[0] | data[1] << 8 , data[2] | data[3] << 8, 0));
}
...
int main(int argc, const char *argv[])
{
	...
	signal(SIGIO, tsl2561_sig);
	
	fd = open("/dev/tsl_dev", O_RDWR);
	if (fd < 0)
		exit(EXIT_FAILURE);
	...
	printf("F_SETFL ret = %d\n", ret);

	tsl2561_thr_value thr;
	...
	//ret = ioctl(fd, TSL_DIS_INT, NULL);
	//printf("TSL_DIS_INT ret = %d\n", ret);

	ret = ioctl(fd, TSL_SET_THR, &thr);
	printf("TSL_SET_THR ret = %d\n", ret);
    
    sleep(1);

	ret = ioctl(fd, TSL_EN_INT, NULL);
	printf("TSL_EN_INT ret = %d\n", ret);
    
 //   sleep(1);
//	ret = ioctl(fd, TSL_GET_THR, &thr);
//	printf("TSL_GET_THR ret = %d %02x %02x %02x %02x\n", ret, thr.data[0], thr.data[1], \
                        thr.data[2], thr.data[3]);
#endif
